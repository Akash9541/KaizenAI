...
export const INDUSTRY_INSIGHTS_UPDATE_INTERVAL_MS =
  90 * 24 * 60 * 60 * 1000;
...
const convertUsdToInr = (value) => Math.round(toNumber(value, 0) * USD_TO_INR_RATE);
...
const hasQuarterlyCadence = (insight) => {
  if (!isValidDateValue(insight?.lastUpdated) || !isValidDateValue(insight?.nextUpdate)) {
    return false;
  }
      ...
      Array.isArray(rawInsights?.salaryRanges) && rawInsights.salaryRanges.length
        ? rawInsights.salaryRanges.map((range, index) => {
            ...
            return {
              ...
              minInr: toNumber(
                range?.minInr,
                convertUsdToInr(minUsd)
              ),
              maxInr: toNumber(
                range?.maxInr,
                convertUsdToInr(maxUsd)
              ),
              medianInr: toNumber(
                range?.medianInr,
                convertUsdToInr(medianUsd)
              ),
            };
          })
